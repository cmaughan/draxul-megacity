...
TEST_CASE("semantic building metrics can bypass clamping", "[megacity]")
{
    ...
    SemanticCityLayoutOptions clamped_config;
    ...
    SemanticCityLayoutOptions unclamped_config = clamped_config;
...
TEST_CASE("road width scale affects unclamped semantic road width", "[megacity]")
{
    ...
    SemanticCityLayoutOptions low_scale_config;
    low_scale_config.clamp_semantic_metrics = false;
    low_scale_config.road_width_scale = 0.25f;

    SemanticCityLayoutOptions high_scale_config = low_scale_config;
...
TEST_CASE("semantic city layout handles very large unclamped lots", "[megacity]")
{
    ...
    SemanticCityLayoutOptions config;
...
TEST_CASE("semantic city layout can hide test entities by source path", "[megacity]")
{
    ...
    SemanticCityLayoutOptions visible_config;
    visible_config.clamp_semantic_metrics = true;
    visible_config.hide_test_entities = false;
    SemanticCityLayoutOptions hidden_config = visible_config;
...
TEST_CASE("semantic megacity layout spirals modules around the largest module", "[megacity]")
{
    ...
    const SemanticMegacityLayout repeated_layout = build_semantic_megacity_layout({ host, app }, config);

    REQUIRE(layout.modules.size() == 3); // central_park + 2 real modules
    REQUIRE(repeated_layout.modules.size() == layout.modules.size());
    ...
    for (size_t module_index = 0; module_index < layout.modules.size(); ++module_index)
    {
        const auto& first = layout.modules[module_index];
        const auto& repeated = repeated_layout.modules[module_index];
        CHECK(repeated.module_path == first.module_path);
        CHECK(repeated.offset == first.offset);
        REQUIRE(repeated.buildings.size() == first.buildings.size());
        for (size_t building_index = 0; building_index < first.buildings.size(); ++building_index)
        {
            CHECK(repeated.buildings[building_index].qualified_name
                == first.buildings[building_index].qualified_name);
            CHECK(repeated.buildings[building_index].source_file_path
                == first.buildings[building_index].source_file_path);
            CHECK(repeated.buildings[building_index].center
                == first.buildings[building_index].center);
        }
    }
}
