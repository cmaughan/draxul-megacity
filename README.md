...
- `product/draxul-code-semantics` projects the raw parse into a neutral `CodeSemanticSnapshot` — repository, module, file, type, function, method, field, and reference nodes — and resolves stable project-module paths for both visualization modes. City projection additionally derives the direct source folder for every building so its layout can preserve the complete repository hierarchy without changing BioView's tissue model.
...
The semantic snapshot is laid out as a city: every source folder that contributes buildings gets a local module spiral, and those districts are recursively composed into labeled parent-folder spirals. Conventional `include/` and `src/` trees are folded into their owning project, so public headers and implementations share one module spiral and neither source root creates deeper boundaries. A tree such as `plugins/alpha/core` therefore remains contained by `plugins/alpha`, which remains contained by the overall `plugins` district instead of collapsing into one giant flat plugin area. Only populated branches appear. Module and folder outlines sit above a shared road layer, and types become procedural buildings whose shape and metrics derive from the code — connected buildings step from 4-sided to 6-sided to 8-sided shells by incident dependency count, and same-footprint structs stack into plate buildings. Dependency references are routed as road-following polylines and emitted into the scene as raised connection strips with a source-to-target gradient. Buildings carry repeated roof-sign rings with class and folder labels; a central park holds a procedurally generated tree with atlas-based PBR leaf cards.
